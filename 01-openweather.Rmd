...
order = c("temp","uvi","clouds","pop","rain")
...
df.dt_label = df.score %>% 
  ...
  dplyr::mutate(dt_label=paste0(dt_label,"W: ",round(wind_speed,0),"m\\s\n")) %>% 
  dplyr::mutate(dt_label=paste0(dt_label,"Regn: ",pop,"%\n")) %>% 
  dplyr::mutate(dt_label=paste0(dt_label,"Skydække: ",clouds,"%\n")) %>% 
...
df.score %>% 
  ...
  labs(x=NULL,y=NULL,title="Sport X Tan & Træn Index") 
